...
class ProcessTrialReminders extends Command
{
    ...
    {
        try {
            Log::info('Starting trial reminders process at: ' . now());
            ...
            while (true) {
                ...
                try {
                    ...
                    Log::channel('trial-reminders')->error('Error in trial reminder process: ' . $e->getMessage());
                }

                // Sleep for 5 minutes before next run
                sleep(300);
            }
            Log::info('Trial reminders process completed at: ' . now());
            return 0;
        } catch (\Exception $e) {
            Log::error('Error in trial reminders process: ' . $e->getMessage());
            return 1;
        }
    }
}
